...
class LeRobotRobodmDataset(Dataset):
    ...
    def __init__(self, trajectory_path: str, delta_timestamps: dict, dataset_metadata: LeRobotDatasetMetadata, return_type: str = "numpy"):
        ...
        image_keys = [k for k in list(self.data.keys()) if 'observation.image_' in k]
        if image_keys:
    ...
    def __getitem__(self, idx):
        ...
        item["observation.image"] = torch.from_numpy(
            self.data["observation.image"][idx]  # Get the temporal sequence for this frame
        ).float()
        
        item["observation.state"] = torch.from_numpy(
            self.data["observation.state"][idx]  # Get the temporal sequence for this frame
        ).float()
        
        item["action"] = torch.from_numpy(
            self.data["action"][idx]  # Get the temporal sequence for this frame
        ).float()
...
if __name__ == "__main__":
    ...
    robodmDataset = LeRobotRobodmDataset(trajectory_path="./tmp/robot_demoNew25650.vla", delta_timestamps=delta_timestamps, dataset_metadata=dataset_metadata)
